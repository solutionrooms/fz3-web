// m4 — the 2.0.x contact solver bit-exact gate.
//
...
// warm-starting) across a real landing-and-sliding settle.
//
// M4FIX — fixedRotation box (invI=0 => never rotates => trig-IMMUNE): PURE bit-exact,
//   every field every step. This is the m4 gate.
// M4ROT — same box free to rotate: the first MEASURE of the trig ceiling (CLAUDE.md
//   rule 5). cos/sin enter once the box rotates during the asymmetric bounce; we assert
//   a bit-exact PREFIX, then a bounded tolerance after (labelled — NOT called "exact").
...
describe("m4 — contact solver (fixedRotation, bit-exact)", () => {
...
describe("m4 — rotating-box landing (trig-ceiling MEASUREMENT, exact-prefix + tolerance)", () => {
  it("M4ROT is bit-exact for a long prefix, then within a tight bound (NOT labelled exact)", () => {
    const rows = runScene(false);
    const g = golden.golden.M4ROT;
    // find first step where any field's bits diverge
    ...
    // enters once the box actually rotates during the bounce). Require a real exact prefix.
    console.log(`[m4 trig measure] M4ROT bit-exact through step ${firstDiverge - 1}; trig diverges at step ${firstDiverge}`);
    expect(firstDiverge, "M4ROT should be bit-exact at least through the pre-bounce phase").toBeGreaterThanOrEqual(45);
