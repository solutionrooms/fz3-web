...
// The bar ROTATES so this is trig-exposed (the first joint case that is) — assert a
// bit-exact PREFIX then a bounded tolerance (rule 5; NOT labelled exact). Also confirms
// the constraint holds: COM stays 1.0 from the pivot.
//
...
// "contraption won't move from rest" fake-friction bug). Assert m_contactCount === 0.
...
describe("m6 — revolute joint (pendulum, trig-measured)", () => {
  it("REV matches the golden for a bit-exact prefix, then within a tight bound", () => {
    ...
    // The constraint must hold exactly for a real prefix (the bar starts axis-aligned;
    // trig only accumulates as it rotates). Require a meaningful exact prefix.
    expect(firstDiverge, "REV should be bit-exact at least through the early swing").toBeGreaterThanOrEqual(10);
...
describe("m6 — collideConnected hotspot (overlapping jointed bodies move freely)", () => {
