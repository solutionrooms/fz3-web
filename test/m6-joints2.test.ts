// m6 — prismatic + distance joints (the other FZ3-used joints).
//
...
// here too); gated bit-exact with the distance invariant. (A rope attached off-COM would
// rotate and be trig-exposed; this scene isolates the distance constraint itself.)
...
function makeWorld(): b2World {
  ...
  const w = new b2World(aabb, new b2Vec2(0, 6), false);
...
describe("m6 — prismatic joint (bit-exact)", () => {
  it("PRIS matches the golden every step (axis-locked => trig-immune)", () => {
    ...
    const check = (i: number): void => {
      ...
      for (let f = 0; f < 6; f++) {
        expect(got[f], `PRIS step ${i} field ${f}`).toBe(norm(g[i].fields[f]));
      }
    };
    check(0);
    for (let i = 1; i <= 60; i++) {
...
describe("m6 — distance joint (bit-exact)", () => {
  it("DIST matches the golden every step + holds the length-2 invariant", () => {
    ...
    const check = (i: number): void => {
      const got = fields(box);
      for (let f = 0; f < 6; f++) {
        expect(got[f], `DIST step ${i} field ${f}`).toBe(norm(g[i].fields[f]));
      }
