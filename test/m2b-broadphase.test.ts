// m2b — SAP broadphase bit-exact gate.
//
// Mirrors tools/oracle/harness-broadphase.as: drives the ported b2BroadPhase through
// CreateProxy / MoveProxy / QueryAABB / DestroyProxy with a plain b2PairCallback, and
// asserts hex16(ours) === golden for the quantized bound arrays (value/proxyId/
// stabbingCount), proxy & pair counts, and query results. Everything is integer-valued
// (quantized endpoints, ids, counts) so this is a PURE bit-exact gate over the
// uint-heavy sort-and-sweep code that feeds narrowphase pair generation at m3.
...
function run(): void {
  ...
  worldAABB.lowerBound.Set(-2500, -2500);
  worldAABB.upperBound.Set(2500, 2500);
...
describe("m2b — SAP broadphase (bit-exact)", () => {
