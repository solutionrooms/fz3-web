// m7 — CCD/TOI bit-exact gate (the hardest hotspot).
//
// Mirrors tools/oracle/harness-toi.as. continuousPhysics ON: a fast bullet (~6
// units/step) that WITHOUT TOI would tunnel clean through a thin static wall is caught
// at the wall by b2World.SolveTOI → b2TimeOfImpact (conservative advancement over GJK
// b2Distance) → b2Island.SolveTOI sub-step. Validates the ENTIRE TOI stack.
//
// M7FIX — fixedRotation bullet, head-on (invI=0 ⇒ angle stays 0 ⇒ the sub-step GetXForm
//   uses cos0/sin0 ⇒ trig-immune): PURE bit-exact gate.
...
describe("m7 — CCD/TOI (fixedRotation bullet, bit-exact)", () => {
  it("M7FIX: a fast bullet is caught at the thin wall (no tunneling) — bit-exact every step", () => {
    ...
    // sanity: TOI actually fired (bullet caught at the wall, not tunneled to x>=5)
...
describe("m7 — off-centre bullet (TOI trig-ceiling MEASUREMENT, exact-prefix + tolerance)", () => {
