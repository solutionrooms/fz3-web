...
// ApplyImpulse WAKES it at step 110. The 7th traced field is isSleeping(0/1), so the
// exact sleep-transition step and the velocity-zeroing at sleep are gated bit-for-bit
// (SB2's stack-settle diverged exactly at the frame allowSleep zeroed a ~1e-15 residual).
//
// Also asserts the faithful 2.0.x wake semantics: Apply* wakes a sleeping body, and (by
// construction of the trap) SetLinearVelocity does NOT — verified in a sub-test.
...
describe("m5 — sleep/wake (bit-exact)", () => {
